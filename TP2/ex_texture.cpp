...
int main(int /*argc*/, char** argv)
{
	...
	// -- Exercice Texture : TODO:

	std::unique_ptr<glimac::Image> pImage = glimac::loadImage("assets/textures/triforce.png");

	if (pImage == nullptr)
	{
		std::cout << "Erreur lors du charggement de l'image" << std::endl;
		return 1;
	}
	...
	std::vector<Vertex2DUV> vertices = {
		Vertex2DUV(glm::vec2(-0.25, -0.25), glm::vec2(0, 1)),
		Vertex2DUV(glm::vec2(0.25, -0.25), glm::vec2(1, 1)),
		Vertex2DUV(glm::vec2(0, 0.25), glm::vec2(0.5, 0))
	};
	...
	while (!glfwWindowShouldClose(window)) {
		...
		*********************************/
		...
		// On reset l'index de la texture à chaque draw
		glUniform1i(uTextureLocation, 0);
